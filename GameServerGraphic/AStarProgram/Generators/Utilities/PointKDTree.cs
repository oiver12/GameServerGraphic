...
namespace Pathfinding {
	...
	public class PointKDTree {
		...
		//	if (data != null) {
		//		var pointv3 = (GameServer.Vector3)point;
		//		for (int i = tree[index].count - 1; i >= 0; i--) {
		//			var dist = (data[i].position - point).sqrMagnitudeLong;
		...
		//				if (conns != null) {
		//					var nodePos = (GameServer.Vector3)data[i].position;
		...
		//						var connectionMidpoint = ((GameServer.Vector3)conns[j].node.position + nodePos) * 0.5f;
