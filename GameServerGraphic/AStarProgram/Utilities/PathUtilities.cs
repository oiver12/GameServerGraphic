		...
		public static void GetPointsAroundPoint (Vector3 center, IRaycastableGraph g, List<Vector3> previousPoints, float radius, float clearanceRadius) {
			...
			for (int i = 0; i < previousPoints.Count; i++) {
				...
				while (true) {
					...
					dir = GameServerRandom.randomSpherePoint(rand) * Mathf.Lerp(newMagn, radius, tests / 5);
					dir.y = 0;
					tests++;
				}
			}
...
//#endif
				...
				for (int i = 0; i < count; i++) {
					...
					while (!worked) {
						...
						float tg = (float)rand.NextDouble() * tot;
				...
				for (int i = 0; i < count; i++) {
					pts.Add((Vector3)nodes[rand.Next(0, nodes.Count)].RandomPointOnSurface());
				}
			}
